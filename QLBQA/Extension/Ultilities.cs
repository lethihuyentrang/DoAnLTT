...
namespace QLBQA.Extension
{
    public static class Ultilities
    {
        ...
        //    }
        //}
        public static string UploadFile(HttpPostedFileBase file, string sDirectory, string newname = null)
        {
            try
            {
                if (newname == null) newname = Path.GetFileName(file.FileName);
                string path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Content", "images", sDirectory);
                CreateIfMissing(path);
                string pathFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Content", "images", sDirectory, newname);
                var supportedTypes = new[] { "jpg", "jpeg", "png", "gif" };
